...
async def get_browsers_stats(
    ...
    **This endpoint allows you to retrieve your email statistics segmented by
    browser type.**  **We only store up to 7 days of email activity in our
    database.** By default, 500 items will be returned per request via the
    Advanced Stats API endpoints.  Advanced Stats provide a more in-depth view
    of your email statistics and the actions taken by your recipients. You can
    ...
    see our [Statistics Overview](https://sendgrid.com/docs/ui/analytics-and-
    ...
    Args:
        ...
        browsers:
            The browsers to get statistics for. You can include up to 10 different
    ...
    [http://api.sendgrid.com/v3/browsers/stats?&browsers=%s&on_behalf_of=%s&limit=%s&offset=%s&aggregated_by=%s&start_date=%s&end_date=%s](
    http://api.sendgrid.com/v3/browsers/stats?&browsers=%s&on_behalf_of=%s&limit=%s&offset=%s&aggregated_by=%s&start_date=%s&end_date=%s)
    ...
    | --- | --- |
    """  # noqa
    url = "http://api.sendgrid.com/v3/browsers/stats"  # noqa
